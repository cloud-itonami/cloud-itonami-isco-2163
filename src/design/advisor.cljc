(ns design.advisor
  "DesignAdvisor — proposes a design operation (draft design concept,
  prepare specification, flag safety compliance concern, request client review)
  for a registered product/garment design project. The advisor is swappable:
  ...
  produces a PROPOSAL — it never writes to the store, never issues a production
  sign-off or certification, and has no notion of project provenance or
  safety/regulatory responsibility; `design.governor` is the independent
  system that decides whether the proposal may proceed, per the itonami
  actor pattern.

  A proposal is a map:
    {:op :draft-design-concept|:prepare-specification|:flag-safety-compliance-issue|:request-client-review
     :effect :propose        ; the advisor NEVER issues production sign-offs or certifications
...
(def ^:private system-prompt
  "You are a product and garment design advisor. Given a design operation
   request, propose an :op, an honest :confidence (0.0-1.0), and a
   :stake (:low/:medium/:high). Never fabricate confidence you don't
   have. You never issue production sign-offs or certify safety compliance —
   those remain the designer's exclusive responsibility.")
