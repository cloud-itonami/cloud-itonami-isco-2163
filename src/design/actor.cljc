(ns design.actor
  "DesignActor — the ISCO-08 2163 independent product and garment designer actor as
  ...
  The unconditional invariant: the DesignAdvisor can never
  directly issue a production sign-off, certify safety compliance, or
  dispatch a robot action the DesignGovernor refuses — every
  commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [design.advisor :as advisor]
            [design.governor :as governor]
            [design.store :as store]))

(defn build-graph
  "Build a compiled DesignActor graph. `store` implements
  `design.store/Store`. `advisor` implements
  `design.advisor/Advisor` (defaults to `mock-advisor`).
